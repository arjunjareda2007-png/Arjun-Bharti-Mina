        ...
        <AnimatePresence mode="popLayout">
          {filteredGallery.map((item, idx) => (
            <motion.div
              ...
              transition={{ duration: 0.4, delay: (idx % 9) * 0.05, ease: CINEMATIC_EASE }}
              onClick={() => {
                hapticSelection();
                openLightbox(item);
              }}
              className="break-inside-avoid group relative rounded-2xl overflow-hidden bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-xs hover:shadow-2xl cursor-pointer transition-all duration-300"
            >
              <img 
                src={item.imageUrl} 
                alt={item.title}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-neutral-950 via-neutral-950/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-4 sm:p-5 text-white">
                <div className="flex items-center justify-between">
                  <span 
                    className="px-2.5 py-0.5 rounded-full text-[10px] font-mono uppercase text-white font-bold"
